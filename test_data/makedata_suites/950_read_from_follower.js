...
const collName = "ReadFromFollowerCollection";
...
(function () {
  return {
    ...
    makeData: function (options, isCluster, isEnterprise, dbCount, loopCount) {
      ...
      // Create collections:
      let coll = db._create(collName, { numberOfShards: 1, replicationFactor: 2, writeConcern: 2 });
      ...
      let ReadFromFollowersSuite = function () {
        ...
        return {
          testSingleDocumentReadsFromFollowers: function () {
            ...
            for (let i = 0; i < nrTries; ++i) {
              let res = db._connection.GET_RAW(
                `/_api/document/${collName}/K${i % 1000}`,
                { "X-Arango-Allow-Dirty-Read": "true" });
          ...
          testBatchDocumentReadsReadFromFollowerJSAPI: function () {
            ...
            for (let i = 0; i < nrTries; ++i) {
              let j = (i + 1) % 1000;
              db._collection(collName).document(["K" + (i % 1000), "K" + j], { allowDirtyReads: true });
            }
            wait(0.3);   // Give statistics time to process ops, need 250ms
