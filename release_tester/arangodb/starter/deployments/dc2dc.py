...
class Dc2Dc(Runner):
    ...
    def test_setup_impl(self):
        rc = self.cluster1['instance'].arangosh.create_test_data("dc2dc (post setup - dc1)")
        if not rc[0]:
            if not self.cfg.verbose:
                print("Script output: ")
                print(rc[1])
            raise Exception("creating test data failed")

        time.sleep(180) # TODO: howto detect dc2dc is completely up and running?
        # exit(0)
        rc = self.cluster1['instance'].arangosh.check_test_data("dc2dc (post setup - dc1)")
        if not rc[0]:
            if not self.cfg.verbose:
                print("Script output: ")
                print(rc[1])
            raise Exception("checking test data on site 1 failed")
        ...
        self.sync_manager.get_sync_tasks(0)
        self.sync_manager.get_sync_tasks(1)
        rc = self.cluster2['instance'].arangosh.check_test_data("dc2dc (post setup - dc2)")
        if not rc[0]:
            if not self.cfg.verbose:
                print("Script output: ")
                print(rc[1])
            raise Exception("checking test data on site 2 failed")
