...
class DatabasePage(NavigationBarPage):
    ...
    def create_new_db(self, db_name, index, cluster, enterprise):
        ...
        self.wait_for_ajax()
        self.print(f'Creating {db_name} database started \n')
        ...
        self.print("fill up all the database details \n")
        if self.version_is_newer_than("3.11.99"):
        ...
        if cluster:
            self.print(f"Cluster deployment selected for {db_name} \n")
            if self.version_is_newer_than("3.11.99"):
            ...
            if enterprise:
                if self.version_is_newer_than("3.11.99"):
                    self.print(f"selecting one shard database for {db_name}\n")
                    if db_name == "OneShard":
                    ...
                    self.print(f"selecting sharded option from drop down using index for {db_name}\n")
                    select_sharded_db = "newSharding"
                    self.locator_finder_by_select(select_sharded_db, index)
                    time.sleep(1)
            else:
                self.print(f"Skipped for Community or < v3.11.99 for {db_name}\n")

        self.print(f"selecting user option from drop down using index for choosing root user for {db_name} \n")
        if self.version_is_newer_than("3.11.99"):
        ...
        time.sleep(4)
        self.print(f"Creating {db_name} database completed \n")
        
        if self.current_package_version() < semver.VersionInfo.parse("3.11.0"):
            self.print(f"Logging into newly created {db_name} database \n")
            ...
            self.print("Database checked and found: ", db_name, "\n")
            time.sleep(4)
            ...
            self.print(f"Logging out from {db_name_sitem} database \n")
            db_id = '//*[@id="dbStatus"]/a[3]/i'
            change_db_sitem = self.locator_finder_by_xpath(db_id)
            change_db_sitem.click()
            time.sleep(4)

            self.print("Re-Login to _system database \n")
    ...
    def test_database_expected_error(self, cluster):
        ...
        self.navbar_goto("databases")
        self.wait_for_ajax()
        self.print("Expected error scenario for the Database name Started. \n")
        create_new_db_btn = self.create_new_db_btn
        create_new_db_btn_sitem = self.locator_finder_by_id(create_new_db_btn)
        create_new_db_btn_sitem.click()
        time.sleep(2)
        ...
        self.print("Closing the database creation \n")
    ...
    def searching_db(self, db_name):
        ...
        try:
            ...
            self.print("Error Occurred! \n")

        self.print("Clearing the search text area \n")
    ...
    def deleting_database(self, db_name):
        ...
        try:
            ...
            self.print(f'{db_name} deleting started \n')
            if db_name == 'OneShard':
                if self.version_is_newer_than("3.11.99"):
            ...
            self.print(f'{db_name} deleting completed \n')
        except TimeoutException:
            self.print('TimeoutException occurred! \n')
            self.print('Info: Database has already been deleted or never created. \n')
        except Exception:
    ...
    def deleting_user(self, username):
        ...
        try:
            self.webdriver.refresh()
            self.print('Selecting user for deletion \n')
            if self.version_is_newer_than("3.11.99"):
            ...
            if self.version_is_newer_than("3.11.99"):
                ...
                self.print(f'Deleting {username} begins \n')
                ...
                self.print(f'Deleting {username} completed \n')
                time.sleep(2)
        except TimeoutException:
            self.print('TimeoutException occurred! \n')
            self.print('Info: User has already been deleted or never created. \n')
        except Exception:
            raise Exception('Critical Error occurred and need manual inspection!! \n')
