...
#        self.webdriver.refresh()
    ...
    def deleting_example_graphs_for_312(self, graph_name):
        """This method will delete all the example graphs for > 3.12.0 graphs"""
        self.webdriver.refresh()
        self.wait_for_ajax()
    ...
    def deleting_example_graphs(self, graph_name):
        """This method will delete all the example graphs with different locators"""
        locators = [
            '//*[@id="dropGraphCollections"]',
            "//*[text()='also drop collections?']"
        ]

        for locator in locators:
            try:
                self.delete_with_locator(graph_name, locator)
                break  # Break if successful
            except Exception as e:
                print(f"An error occurred: {e} trying different xpath locator \n")
                continue  # Try the next locator if unsuccessful
    
    def delete_with_locator(self, graph_name, locator):
        """This method contains common deletion logic with a specific locator"""
        print(f"Deleting {graph_name} Graph with locator: {locator} \n")
        self.select_graph_page()
        self.webdriver.refresh()
        ...
        # Use the provided locator for further deletion steps
        delete_with_collection_sitem = self.locator_finder_by_xpath(locator)
        delete_with_collection_sitem.click()
