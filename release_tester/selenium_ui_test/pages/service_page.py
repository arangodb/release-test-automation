...
class ServicePage(NavigationBarPage):
    ...
    def service_search_option(self, search_keyword):
        ...
        self.print(f'Searching for {search_keyword} \n')
    ...
    def service_category_option(self):
        ...
        self.print('Selecting category options \n')
    ...
    def select_category_option_from_list(self, category):
        ...
        if category == "connector":
            self.print(f"Selecting {category} category from the drop-down menu \n")
        ...
        if self.version_is_newer_than("3.11.99"):
            self.print("prometheus and monitoring is gone from >3.11.99 \n")
        else:
    ...
    def select_demo_geo_s2_service(self):
        ...
        self.print('Selecting demo_geo_s2 service \n')
    ...
    def install_demo_geo_s2_service(self, mount_path, ui_data_dir):
        ...
        self.print('Installing demo_geo_s2 service \n')
        service = 'installService'
        service_sitem = self.locator_finder_by_id(service)
        service_sitem.click()
        time.sleep(2)

        self.print(f'Selecting service mount point at {mount_path} \n')
        ...
        self.print('Selecting install button \n')
        ...
        if self.version_is_newer_than("3.10.100"):
            ...
            try:
                ...
                if success_sitem == 'demo-geo-s2':
                    self.print(f"{success_sitem} has been successfully created \n")
                    status = True
                else:
                    self.print('Could not locate the desired service! refreshing the UI \n')
                    self.webdriver.refresh()
                    time.sleep(1)
                    success_sitem = self.locator_finder_by_xpath(success).text
                    if success_sitem == 'demo-geo-s2':
                ...
                if status:
                    ...
                    if neighbourhoods_collection_sitem.text == "neighborhoods":
                        self.print("open it and populate necessary data into it \n")
                        ...
                        self.print('select upload button \n')
                        ...
                        self.print(f'Providing neighborhood collection path {path1} \n')
                    ...
                    if restaurants_collection_sitem.text == 'restaurants':
                        ...
                        self.print(f'Providing restaurants collection path {path2} \n')
                        choose_file_btn = 'importDocuments'
                        choose_file_btn_sitem = self.locator_finder_by_id(choose_file_btn)
    ...
    def check_demo_geo_s2_service_api(self):
        ...
        self.print('Selecting service API \n')
        api = 'service-api'
        self.locator_finder_by_id(api).click()
        time.sleep(2)

        self.print("Changing view to JSON form \n")
        json = 'jsonLink'
        self.locator_finder_by_id(json).click()
        time.sleep(3)

        self.print('get back to swagger view \n')
        json = 'jsonLink'
        self.locator_finder_by_id(json).click()

    def checking_function_for_fox_leaflet(self, id_list):
        """this method will take list and check according to that list"""
        self.print(f'There are total {len(id_list)} Foxx leaflets \n')
        i = 0
        while i < len(id_list):
            self.print(f'Checking Foxx leaflet number {i}\n')
    ...
    def inspect_demo_geo_foxx_leaflet_iframe(self):
        ...
        if self.version_is_newer_than("3.11.100"):
            ...
            self.print("Getting out of IFrame \n")
    ...
    def inspect_foxx_leaflet_iframe(self):
        """Checking iframe elements of foxx and leaflets"""
        if self.version_is_newer_than("3.11.100"):
            self.print('skipped inspect_foxx_leaflet_iframe() \n')
        else:
            self.print('Switching to IFrame \n')
            iframe_id = 'swaggerIframe'
            self.webdriver.switch_to.frame(self.locator_finder_by_id(iframe_id))
    ...
    def install_demo_graph_hql_service(self, mount_path):
        ...
        self.print('Selecting graphql service \n')
        ...
        # self.print('Checking graphql Github link \n')
        ...
        # self.print('Checking graphql documentation link \n')
        ...
        self.print('Install the service \n')
        install_btn = 'modalButton1'
        install_btn_sitem = self.locator_finder_by_id(install_btn)
        install_btn_sitem.click()
        time.sleep(6)
        ...
        if self.version_is_newer_than("3.10.100"):
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    time.sleep(2)
                    self.print(f"Attempt {attempt}: Element not found. Retrying...")
                    if attempt == max_retries:
                        self.print("Maximum retries reached. Exiting.")
                        raise  # Re-raise the exception if max retries reached
                    else:
                        status = False
                        self.print("Refreshing the UI \n")
                        self.webdriver.refresh()
                        time.sleep(2)
                        self.navbar_goto("services")
                        self.wait_for_ajax()
            ...
            if status:
                self.print('Opening graphql interface \n')
                ...
                self.print('Switching to code mirror windows of graphql \n')
                self.webdriver.switch_to.window(self.webdriver.window_handles[1])
                self.wait_for_ajax()
                ...
                self.print('Return back to original window \n')
                self.webdriver.close() # closes the browser active window
                self.webdriver.switch_to.window(self.webdriver.window_handles[0])

                self.wait_for_ajax()
                self.print('Checking API tab of graphql service \n')
                ...
                if self.version_is_newer_than("3.10.100"):
                    self.print('skipped inspecting graphql service \n')
                else:
                    self.print('Selecting Swagger view \n')
                    ...
                    try:
                        ...
                        self.print("Error occurred while inspecting API tab of graphql service")
                        self.print('Getting out of IFrame \n')
                        self.webdriver.switch_to.default_content()
                        time.sleep(1)
    ...
    def replace_service(self):
        ...
        self.print("Replacing demo_geo_s2 service with demo-graphql service \n")
        ...
        if self.version_is_newer_than("3.11.100"):
            ...
            self.print("Service successfully replaced \n")

    def select_service_settings(self):
        """Selecting service settings tab"""
        self.print("Selecting settings options \n")
        ...
        if service_name == '/geo':
            ...
            try:
                ...
                if service_sitem == '/geo':
                    self.print(f'{service_sitem} service has been found and ready to delete \n')
                    ...
                    self.webdriver.refresh()
                    self.print(f'{service_sitem} service has been deleted successfully \n')
                        
            except TimeoutException:
                self.print('TimeoutException occurred! \n')
                self.print(f'Info: {service_name} has already been deleted or never created. \n')
            except Exception:
