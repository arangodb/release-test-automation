...
class QueryTestSuite(BaseSeleniumTestSuite):
    ...
    def test_query(self):
        ...
        self.print("Checking saved query feature \n")
        ...
        if query_page.current_package_version() >= semver.VersionInfo.parse("3.10.0"):
            ...
            self.print("Importing IMDB collections \n")
            query_page.import_collections(self.restore, self.ui_data_dir, self.is_cluster)

            self.print("Selecting Query page for basic CRUD operation \n")
            query_page.selecting_query_page()

            self.print("Executing insert query \n")
            query_page.execute_insert_query()

            self.print("Profiling current query \n")
            query_page.profile_query()
            self.print("Explaining current query \n")
            query_page.explain_query()
            self.print("Debug packaged downloading for the current query \n")
            query_page.debug_package_download()
            self.print("Removing all query results \n")
            ...
            # TODO: self.print("Executing spot light functionality \n")
            # query_page.spot_light_function('COUNT')  # can be used for search different keyword

            self.print("Executing read query\n")
            query_page.execute_read_query()

            self.print("Updating documents\n")
            query_page.update_documents()
            self.print("Executing query with bind parameters \n")
            query_page.bind_parameters_query()

            self.print("Executing example graph query \n")
            graph = GraphExample.WORLD
            query_page.navbar_goto("graphs")
            self.webdriver.refresh()
            self.print("Creating '%s' Graph" % get_graph(graph).get_name())
            ...
            graph_page.create_graph(graph, self.importer, self.ui_data_dir)
            graph_page.check_required_collections(graph)
            self.print("Executing City Graph query \n")
            ...
            # self.print("Importing new queries \n") # untill the fix arrive
            # query_page.import_queries(str(self.ui_data_dir / "ui_data" / "query_page" / "imported_query.json"))
            self.print("Saving Current query as custom query\n")
            query_page.custom_query()
            self.print("Changing the number of results from 1000 to 100\n")
            query_page.number_of_results()

            self.print("Deleting collections begins \n")
            query_page.delete_all_collections()
            self.print("Deleting collections completed \n")

        del query_page
        self.print("---------Checking Query completed--------- \n")
