...
class VectorIndexStoredValuesTestSuite(APITestSuite):
    ...
    def create_collection_with_index(self):
        """create collection with docs and (trained) vector index"""
        if self.collection == RTA_COLLECTION:
            ...
            response_codes.append(self.execute_request(request_data)["code"])
            # create vector index with stored values. Always build in the
            # foreground (inBackground: false) so the request only returns once the
            # index has finished training; this trains the index synchronously and
            # avoids any separate wait for trainingState across all versions.
            request_data = self.requests_data["create_vector_index_with_stored_values"]
            request_data = rh.update_request_data(request_data, self.collection)
            request_data["payload"]["inBackground"] = False
            response_codes.append(self.execute_request(request_data)["code"])
